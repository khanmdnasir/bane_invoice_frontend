  ...
  return (
    <div>
      <div className="bg-nav_color flex justify-between items-center h-16 px-4">
        ...
        <div
          ref={navButtonRef}
          className="absolute z-10 left-0 top-[120px] md:top-0 md:relative md:flex md:grow md:justify-start md:text-white md:font-medium md:h-16 md:leading-custom"
        >
          {navItems.map((item, index) => (
            <div
              className="hidden md:inline-block md:hover:bg-hoverColor"
              key={index}
            >
              {item.sublinks.length > 0 ? (
                <div
                  className={`md:hover:bg-hoverColor ${
                    navOpen ? "md:bg-hoverColor" : ""
                  }`}
                >
                  <Link
                    key={item.name}
                    className={`md:px-3.5 md:relative md:hover:bg-hoverColor ${
                      navOpen ? "md:bg-hoverColor" : ""
                    }`}
                    to={item.path}
                  ...
                  {navOpen && (
                    <ul className="md:bg-white md:py-2 md:rounded md:border-2 md:border-solid md:border-gray-300 md:absolute md:left-[187px] md:top-[70px] md:shadow-lg md:leading-10 md:w-40">
        ...
        <div ref={dropdownRef} className="md:hidden">
          <div>
            <button onClick={() => setResponsiveOpen(!responsiveOpen)}>
              <FontAwesomeIcon
                className="mr-4 px-4 py-2 rounded-full relative hover:bg-hoverColor md:hidden cursor-pointer text-white text-lg"
              ...
              <ul className="bg-white py-2 rounded border-2 border-solid border-gray-300 absolute right-2 top-[70px] shadow-lg leading-10 w-60">
                ...
                <li className="px-2.5 py-2 text-sm hover:bg-gray-200">
                  <button
                    ...
                    className="bg-userBgColor p-1.5 rounded-full mr-4"
                  >
          ...
          {searchOpen && (
            <div
              ref={searchButtonRef}
              className="bg-gray-100 text-black absolute px-2 py-2 shadow-md rounded w-80 h-fit right-2 top-[70px]"
            >
              <Search />
            </div>
          )}
          ...
          {notificationOpen && (
            <div
              ...
              className="bg-white text-black border-2 border-solid border-gray-300 absolute shadow-md rounded w-96 h-fit right-2 top-[70px]"
            >
          ...
          {profileOpen && (
            <div
              ...
              className="bg-white text-black border-2 border-solid border-gray-300 absolute shadow-md rounded w-60 h-fit right-2 top-[70px]"
            >
              <ProfileDropDown />
            </div>
          )}
        ...
        <div className="hidden md:flex gap-8 md:text-white md:items-center">
          <div>
            ...
            {createOpen && (
              <ul
                ref={createButtonRef}
                className="bg-white py-2 rounded border-2 border-solid border-gray-300 absolute right-[200px] top-[70px] shadow-lg leading-10 w-48"
              >
                <li className=" text-xs font-normal py-2 px-4 text-gray-500">
                  Create New
                </li>
          ...
          <div>
            ...
            {searchOpen && (
              <div
                ref={searchButtonRef}
                className="bg-gray-100 text-black absolute px-2 py-2 shadow-md rounded w-80 h-fit right-[30px] top-[70px]"
              >
                <Search />
              </div>
            )}
          ...
          <div ref={notificationButtonRef}>
            ...
            {notificationOpen && (
              <div
                ref={notificationButtonRef}
                className="bg-white text-black border-2 border-solid border-gray-300 absolute shadow-md rounded w-96 h-fit right-[125px] top-[70px]"
              >
                <Notification setNotificationOpen={setNotificationOpen} />
              </div>
            )}
          ...
          <div
            ref={profileButtonRef}
            className="bg-userBgColor p-2 rounded-full"
          >
            <button
            ...
            {profileOpen && (
              <div
                ref={profileButtonRef}
                className="bg-white text-black border-2 border-solid border-gray-300 absolute shadow-md rounded w-60 h-fit right-[20px] top-[70px]"
              >
                <ProfileDropDown />
              </div>
            )}
